...
if(data.freq == 1) {
    ...
    load('data/baboons.RData')
} else if(data.freq == 5) {
    set.seed(1107)
    dat <- "baboon-thin5"
    load('data/baboons-thin5.RData')
} else if(data.freq == 20) {
    set.seed(1107)
    dat <- "baboon-thin20"
...
if(data.freq == 1) {
    ...
    #proposal SDs
    prop.alpha <- 0.002 #0.001
    prop.sigma <- 0.05 #0.025
    prop.slow <- 0.01 #0.0025
    prop.fast <- 0.1 #0.050
...
#number of traj updates per MCMC iteration
